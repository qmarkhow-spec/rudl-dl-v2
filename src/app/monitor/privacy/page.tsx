...
import MonitorPrivacyClient from './MonitorPrivacyClient';
...
export default async function MonitorPrivacyPage() {
  ...
  const initialData = await fetchTelegramSettings(DB, uid);

  return (
    <section className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <div>
        <h2 className="text-lg font-semibold text-gray-900">{t('monitor.privacy.title')}</h2>
        <p className="mt-1 text-sm text-gray-600">{t('monitor.privacy.description')}</p>
      </div>
      <MonitorPrivacyClient initialData={initialData} />
    </section>
  );
}
